	...
	*/
	...
	public:
		...
		*/
    	...
    	//コマンドをtinypowerに送るためのrosの時間の関数
    	ros::Timer command_timer_;
		void timer_callback(const ros::TimerEvent& e);
        boost::mutex send_sync_;
        //boost::condition send_state_;
		double command_timer_interval;
    //ポートが開いているかどうかのフラグ
		bool open_flag;
	...
	private:
		...
		void write_ok(const boost::system::error_code& e, size_t size);
		
		bool is_connect_;
		std::queue<std::string> que_string;
		// 最新版の受信データ
		std::string readData;
		// 区切り文字
	};
